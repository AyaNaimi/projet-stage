  ...
  return (
    ...
    >
      ...
      >
        ...
        />
        <Form.Label
          style={{ flex: "1", marginRight: "5px", marginLeft: "7px" }}
        >
          Calibre
        </Form.Label>
        <Form.Select
          key={calibreList.length}
          style={{ flex: "2" }}
          name="calibre_id"
          ...
          <Form>
            <Form.Group>
              ...
              <Form.Control
                ...
                onChange={(e) =>
                  setNewCategory({ ...newCategory, categorie: e.target.value })
                }
              />
            </Form.Group>
      ...
      <Modal
        show={showEditClibreModal}
        onHide={() => setShowEditClibreModal(false)}
      >
        <Modal.Header closeButton>
          <Modal.Title></Modal.Title>
        </Modal.Header>
        ...
        >
          ...
          {(() => {
            ...
            if (familles.length === 0) {
              return <option disabled>Aucune famille disponible</option>;
            }
            return familles.map((category) => (
                  ...
                  <tbody>
                    {categories
                      .filter(
                        (cat) =>
                          cat.parent_id !== null &&
                          cat.parent_id === Number(formData.categorie_id),
                      )
                      .map((categ) => (
                        <tr key={categ.id}>
                          <td>{categ.id}</td>
      ...
      <Modal
        show={showEditSousModal}
        onHide={() => setShowEditSousModal(false)}
      >
        <Modal.Header closeButton>
          <Modal.Title></Modal.Title>
        </Modal.Header>
        ...
        <Form.Group className="d-flex justify-content-center">
          ...
          <Fab
            variant="extended"
            className="btn-sm FabAnnule mb-2 mx-2"
            onClick={() => setShowEditSousModal(false)}
          >
            Annuler
          </Fab>
        </Form.Group>
        ...
        >
          ...
          {(() => {
            const sousCats =
              categories && Array.isArray(categories)
                ? categories.filter(
                    (cat) => cat.parent_id === Number(formData.categorie_id),
                  )
                : [];
            if (sousCats.length === 0) {
              return <option disabled>Aucun type disponible</option>;
            }
            return sousCats.map((category) => (
              <option key={category.id} value={category.id}>
                {category.categorie}
              </option>
            ));
        ...
        <div style={{ flex: "2" }}>
          {["kg", "unite", "litre", "kg/unite", "M"].map((value) => (
            <Form.Check
              key={value}
              type="radio"
              label={value === "unite" ? "Unite" : value}
              name="type_quantite"
              value={value}
              onChange={handleChange}
              checked={formData.type_quantite === value}
              isInvalid={!!errors.type_quantite}
            />
          ))}
        </div>
      </Form.Group>
      ...
      >
        <div className="table-responsive" style={{ padding: "0" }}>
          <table
            ...
            style={{ width: "100%", marginTop: "2px", padding: "0" }}
          >
            <thead>
              <tr>
            ...
            <tbody>
              {selectedProductsDataRep?.map((productData, index) => (
                <tr key={productData.id || `temp-${index}`}>
                  ...
                  <td style={{ backgroundColor: "white" }}>
                    <Form.Control
                      ...
                      onChange={(e) =>
                        handleInputChangeRep(index, "date_fin", e.target.value)
                      }
                  ...
                  <td style={{ backgroundColor: "white" }}>
                    <Form.Select
                      ...
                      onChange={(e) =>
                        handleInputChangeRep(index, "type", e.target.value)
                      }
                  ...
                  <td style={{ backgroundColor: "white", width: "10%" }}>
                    <a href="#">
                      <FontAwesomeIcon
                        color="red"
                        onClick={() =>
                          handleDeleteProductRap(index, productData.id)
                        }
                        icon={faTrash}
                      />
                    </a>
