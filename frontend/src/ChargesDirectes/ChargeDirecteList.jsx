...
const ChargeDirecteList = () => {
  ...
  const fetchData = async () => {
    try {
      const [prodRes, catRes] = await Promise.all([
        axiosInstance.get('/api/produits', { params: { directes: 1 } }).catch(() => ({ data: {} })),
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    const url = `/api/produits/${formData.id}/charges-directes`;
    ...
    try {
      ...
      fetchData();
      closeForm();
      Swal.fire("Succès", "Charges directes modifiées avec succès.", "success");
    } catch (error) {
  ...
  const handleDelete = (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          await axiosInstance.patch(`/api/produits/${id}/charges-directes`, { cout_horaire_mod: 0, temps_production: 0 });
          fetchData();
      ...
      if (result.isConfirmed) {
        try {
          ...
          await Promise.all(selectedItems.map(id => 
            axiosInstance.patch(`/api/produits/${id}/charges-directes`, { cout_horaire_mod: 0, temps_production: 0 })
          ));
          fetchData();
          setSelectedItems([]);
          Swal.fire("Réinitialisé !", "Les charges ont été remises à zéro.", "success");
