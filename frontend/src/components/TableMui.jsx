...
export default function TableMui({
  ...
  handleSelectItem,
  handleSelectAll,   // nouveau : pour la case « tout sélectionner »
  width,
  getRowStyle, // Ajout du support pour getRowStyle
  addButtonText,
  ExportButtons,
  totalPages,        // nouveau : pages réelles issues du backend
}) {
  // Gestion de la visibilité des colonnes en interne
  const defaultVisibility = React.useMemo(() => {
    const vis = {};
    columns.forEach(col => {
      vis[col.id || col.key] = col.hiddenByDefault ? false : true;
    });
    return vis;
  }, [columns]);
  ...
  const totalItemCount = Array.isArray(produitsFiltres) && produitsFiltres.length > 0
    ? produitsFiltres.length
    : rows.length;
  const computedTotalPages = totalPages ?? Math.ceil(totalItemCount / rowsPerPage);
  const safeTotalPages = Number.isFinite(computedTotalPages) && computedTotalPages > 0 ? computedTotalPages : 1;
      ...
      >
        <TableContainer sx={{ maxHeight }}>
          <Table sx={tableStyles} stickyHeader aria-label="sticky table">
            <TableHead>
              <TableRow>
                {/* Colonne checkbox — affichée uniquement si handleSelectItem est fourni */}
                {handleSelectItem && handleSelectAll && (
                  <TableCell
                    sx={{
                      ...headerCellStyles,
                      width: 48,
                  ...
                  >
                    <input
                      ...
                      style={{ width: 16, height: 16, cursor: 'pointer', accentColor: '#00afaa' }}
                ...
                return (
                  <React.Fragment key={row.id || row.code || idx}>
                    <TableRow
                      ...
                      sx={{
                        '& td': { borderRight: '1px solid #e0e0e0', padding: '0.75rem 1rem', textAlign: 'center' },
                        backgroundColor: isRowSelected ? '#f0fdf4' : 'inherit',
                      }}
                    >
                      {/* Cellule checkbox par ligne */}
                      {handleSelectItem && handleSelectAll && (
                        <TableCell
                          sx={{
