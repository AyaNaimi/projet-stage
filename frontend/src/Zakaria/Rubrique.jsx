...
import { TextField, FormControl, Autocomplete, Fab } from "@mui/material";
...
const Rubrique = () => {
  ...
  useEffect(() => {
    const fetchPerms = async () => {
      try {
        ...
        const roles = Array.isArray(resp.data)
          ? resp.data[0]?.roles
          : resp.data?.roles;
        const perms =
  ...
  useEffect(() => {
    const initializeData = async () => {
      setIsLoading(true);
      try {
        const response = await axiosInstance.get("/api/group-rubriques");
  ...
  useEffect(() => {
    const memos = new Set(
      rubriques.map((rubrique) => rubrique.memo).filter(Boolean),
    );
    ...
    const loadRubriques = async () => {
      ...
      try {
        const response = await axiosInstance.get(
          `/api/rubriques?group=${selectedGroup.id}`,
        );
  ...
  const applyFilters = useCallback(
    (rubriques) => {
      return rubriques.filter((rubrique) => {
        const codeValue = rubrique.code
          ? rubrique.code.toString().toLowerCase()
          : "";
        const typeValue = rubrique.type_rubrique
          ? rubrique.type_rubrique.toString().toLowerCase()
          : "";
        const matchesCode =
          !filters.code || codeValue.includes(filters.code.toLowerCase());
        const matchesType =
          !filters.type || typeValue.includes(filters.type.toLowerCase());
  ...
  const getRubriquesByMemo = useCallback(
    (memo) => {
  ...
  const handleSubmit = async (formData) => {
    try {
      ...
      if (editingRubrique) {
        ...
        response = await axiosInstance.post("/api/rubriques", formData);
  ...
  const handleDelete = (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          await axiosInstance.delete(`/api/rubriques/${id}`);
  ...
  const handlePrint = useCallback(() => {
    ...
    const tableColumn = ["Désignation", "Code", "Type", "Mémo", "Valeur"];
    const tableRows = rubriques.map((rubrique) => [
      rubrique.designation,
      rubrique.code,
      rubrique.type_rubrique,
  ...
  const handleAddGroupe = async () => {
    try {
      ...
      const response = await axios.post(
        "http://127.0.0.1:8000/api/group-rubriques",
        {
    ...
    if (result.isConfirmed) {
      try {
        const response = await axios.delete(
          `http://127.0.0.1:8000/api/group-rubriques/${groupId}`,
        );
      ...
      try {
        await Promise.all(
          selectedRubriques.map((id) =>
            axios.delete(`http://127.0.0.1:8000/api/rubriques/${id}`),
          ),
        );

        setRubriques((prev) =>
          prev.filter((rubrique) => !selectedRubriques.includes(rubrique.id)),
        );
        setSelectedRubriques([]);
    ...
    (rubrique) => (
      <div className="p-4">
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            <p>
              <strong>Code:</strong> {rubrique.code}
            </p>
            <p>
              <strong>Intitulé:</strong> {rubrique.intitule}
            </p>
            <p>
              <strong>Type:</strong> {rubrique.type_rubrique}
            </p>
          </div>
          <div>
            <p>
              <strong>Formule:</strong> {rubrique.formule}
            </p>
            <p>
              <strong>Mémo:</strong> {rubrique.memo}
            </p>
          </div>
        </div>
      </div>
                  ...
                  <div className="groups-header">
                    <span>
                      ...
                      <FontAwesomeIcon
                        ...
                        style={{
                          cursor: canCreateGroupRubrique
                            ? "pointer"
                            : "not-allowed",
                          marginTop: "2%",
                          marginLeft: "57%",
                          opacity: canCreateGroupRubrique ? 1 : 0.5,
                        }}
                        onClick={() => {
                          if (!canCreateGroupRubrique) return;
              ...
              >
                      ...
                      <div className="filter-group">
                        ...
                        <div className="filter-input-wrapper">
                          ...
                          >
                            ...
                            {[
                              ...new Set(
                                rubriques.map((item) => item.type_rubrique),
                              ),
                            ].map((type, idx) => (
                ...
                <div className="mt-4">
                    ...
                    <div className="d-flex justify-content-between align-items-center">
                      <div>
                        ...
                        <p className="section-description text-muted mb-0">
                          {selectedGroup ? selectedGroup.designation : "Aucun"}{" "}
                          - Groupe sélectionné
                        </p>
                      </div>
                      ...
                      <div className="d-flex align-items-center">
                        <FontAwesomeIcon
                          onClick={() =>
                            handleFiltersToggle &&
                            handleFiltersToggle(!showFilters)
                          }
                  ...
                  >
                    ...
                    {selectedGroup &&
                      Array.from(uniqueMemos).map((memo) => (
                        <Tab
                          key={memo}
                          eventKey={memo}
                          title={memo}
                        ...
                        >
                          <ExpandRTable
                            ...
                            data={getRubriquesByMemo(memo).filter(
                              (rubrique) => {
                                const codeValue = rubrique.code
                                  ? rubrique.code.toString().toLowerCase()
                                  : "";
                            ...
                            highlightText={(text, searchTerm) => {
                              ...
                              const searchTermLower = searchTerm.toLowerCase();
                              if (
                                !textStr.toLowerCase().includes(searchTermLower)
                              )
                                return textStr;
                              const parts = textStr.split(
                                new RegExp(`(${searchTerm})`, "gi"),
