...
const ProduitList = () => {
  ...
  const handleDeleteSelected = async () => {
    ...
    if (result.isConfirmed) {
      try {
        ...
        await axiosInstance.delete(`/api/deleteSelectProd`, {
  ...
  const handleSubmit = async (e) => {
    ...
    if (editingProduit) {
      requestData = {
        ...
        emballage_primaire_label: formData.emballage_primaire_label || "",
        emballage_secondaire_label: formData.emballage_secondaire_label || "",
        etiquette_label: formData.etiquette_label || "",
      ...
      if (formData.logoP) {
        ...
        try {
          const response = await fetch(
            `${import.meta.env.VITE_API_URL}/api/produit/${editingProduit.id}/update-logo`,
            {
              method: "POST",
      ...
      formDatad.append("emballage_primaire_label", formData.emballage_primaire_label || "");
      formDatad.append("emballage_secondaire_label", formData.emballage_secondaire_label || "");
      formDatad.append("etiquette_label", formData.etiquette_label || "");
  ...
  const handleDeletecatgeorie = async (categorieId) => {
    try {
      await axios.delete(
        `${import.meta.env.VITE_API_URL}/api/categories/${categorieId}`,
      );
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await axios.post(
        `${import.meta.env.VITE_API_URL}/api/categories/${selectedCategoryId.id}`,
        formData,
      );
  ...
  const handleSaveClibre = async () => {
    ...
    try {
      await axios.put(
        `${import.meta.env.VITE_API_URL}/api/calibres/${selectedCategoryId.id}`,
        {
  ...
  const handleAddSousCategory = async () => {
    try {
      ...
      const formData = new FormData();
      formData.append("categorie", newCategory.categorie);
      // Only append idCatMer if it's a real value (not null/undefined/empty)
      if (idSucategorie !== null && idSucategorie !== undefined && idSucategorie !== '') {
        formData.append("idCatMer", idSucategorie);
      }

      // Only append image if a real file was selected
  ...
  const handleAddClibre = async () => {
    try {
      ...
      const response = await axios.post(
        `${import.meta.env.VITE_API_URL}/api/calibres`,
        formData,
        {
          headers: {
            "Content-Type": "multipart/form-data",
