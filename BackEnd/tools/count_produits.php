...
if (!isset($env['DB_CONNECTION']) || $env['DB_CONNECTION'] !== 'mysql') { fwrite(STDERR, "Only MySQL supported\n"); exit(3); }
...
$stmt = $pdo->query('SELECT COUNT(*) AS c FROM `produits`');
$row = $stmt->fetch(PDO::FETCH_ASSOC);
echo "PRODUITS_DB_COUNT=" . ($row['c'] ?? 0) . "\n";
